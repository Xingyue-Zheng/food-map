...
> 面向江宁大学城学生的选餐决策工具，解决"不知道吃什么"这件每天都要发生一次的小事。
> 店铺数据来自嬉游旅行指南《人均30+，来这条美食街吃到扶墙》一文的实地探店记录。
...
南京义乌小商品城（江宁区，地铁 1 号线龙眠大道 2 号口）紧靠江宁大学城，一条美食街上聚集了大量餐饮商户，但学生普遍面临"吃什么"的决策困境：信息分散在各个平台，评价体系面向大众而非学生，缺少"人均 15 以内 / 步行 5 分钟 / 有学生折扣"这类真正影响学生决策的维度。
...
顶栏「逛吃分区」切换对以上四个模块同时生效，分区按这条街的实际动线划分：街口·地铁出口 / 奶茶一条街 / 本土连锁区 / 小吃中段 / 街尾·尽头。
...
- 店铺数据为 **22 条硬编码数据**，写在 `index.html` 的 `D` 数组里。店名、招牌单品、价格、分区来自上述探店文章，属真实商户信息；但文章发布于 2021 年 2 月，商户可能已变动
- 评分、评论数、热度值、学生评价、营业时间、"实时更新" **均为模拟填充**，文章未提供这些数据，不存在数据源或后端
- "导航前往"只弹提示，未接入任何地图服务
- 收藏状态存在内存中，刷新即丢失
